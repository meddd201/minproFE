...
import useEditOrgaizer from "@/hooks/api/profile/useEditOrgaizer";
...
import { EditOrganizerSchema } from "../schema";
...
const EditOrganizerForm = () => {
  const { mutateAsync: EditOrg, isPending } = useEditOrgaizer();
  const {
    data: initialData,
  ...
  const initialValues = {
    ...
    bankTarget: initialData?.data.bankTarget || "",
    paymentTarget: initialData?.data.paymentTarget || "",
    organizerPict: null,
  };

  const formik = useFormik({
    initialValues: initialValues,
    validationSchema: EditOrganizerSchema,
    enableReinitialize: true,
    onSubmit: async (values) => {
      await EditOrg(values);
    },
  });
  ...
  if (initialPending) return <Loading className="container mx-auto h-[80vh]" />;
  if (initialError) return redirect("/organization");
  if (!initialData) return redirect("/organization");
  ...
  return (
    ...
    >
      <div className="grid gap-2">
        ...
        {(selectedImage || initialData.data.organizerPicture) && (
            ...
            <div className="relative h-[200px] w-[200px] overflow-hidden rounded-md border-2 border-dashed border-gray-300">
              <Image
                src={
                  selectedImage
                    ? selectedImage
                    : initialData.data.organizerPicture!
                }
                alt="organizerPict"
                fill
